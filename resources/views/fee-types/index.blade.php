                    ...
                    <table class="table-auto w-full border-collapse border border-black">
                        ...
                        <tbody>
                            ...
                            @foreach($categories as $category)
                                ...
                                <tr class="border-black">
                                    <td rowspan="{{ count($category->feeTypes) + 1 }}"
                                        class="border-black border px-4 py-2">{{ $category->name }} ({{ $category->id }})
                                    </td>
                                </tr>
                                @foreach($category->feeTypes as $feeType)
                                    <tr class="border-black">
                                        {{-- <td class="border-black border px-4 py-2">{{$feeType->feeCategory->type}}</td>--}}
                                        <td class="border-black border px-4 py-2">{{$feeType->type}} ({{ $feeType->id }})</td>
                                        <td class="border-black border px-4 py-2 text-right">
                                            {{number_format($feeType->amount, 2)}}
                                        </td>
                                        <td class="border-black border px-4 py-2 text-right">{{number_format($feeType->hif, 2)}}
                                        </td>
                                        <td class="border-black border px-4 py-2 text-center print:hidden">
                                            <a href="{{ route('feeType.edit', $feeType->id) }}"
                                                class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                        </td>
                                        {{-- <td class="border-black border px-4 py-2 text-center print:hidden">--}}
                                            {{-- <form action="{{ route('feeType.destroy', $feeType->id) }}" method="POST"
                                                class="inline-block">--}}
                                                {{-- @csrf--}}
                                                {{-- @method('DELETE')--}}
                                                {{-- <button type="submit"
