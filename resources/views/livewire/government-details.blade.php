<div>
    <div class="mb-4">
        ...
        <div wire:ignore class="mt-2">
            <select class="w-full border rounded-md text-gray-700 focus:outline-none focus:border-blue-500"
                name="government_department_id" id="select2">
                <option value="">None</option>
                @if(request()->routeIs('patient.edit'))
                    @foreach(\App\Models\GovernmentDepartment::orderBy('name', 'ASC')->get() as $dept)
                        <option value="{{$dept->id}}" @if($patient->government_department_id == $dept->id) selected @endif>
                            {{ $dept->name }}</option>
                    @endforeach
                @else
                    @foreach(\App\Models\GovernmentDepartment::orderBy('name', 'ASC')->get() as $dept)
    ...
    @if($isGovernment == 95)
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
                <label for="sehat_sahulat_visit_no" class="block text-red-500 font-bold mb-2">Visit ID # Sehat Sahulat
                    Program</label>
                <input type="text" name="sehat_sahulat_visit_no" id="sehat_sahulat_visit_no"
                    value="{{ old('sehat_sahulat_visit_no') }}"
                    class="w-full px-3 py-2 border rounded-md text-gray-700 focus:outline-none focus:border-blue-500"
                    placeholder="Enter Visit ID">
            </div>

            <div>
                <label for="sehat_sahulat_patient_id" class="block text-red-500 font-bold mb-2">Patient ID Sehat Sahulat
                    Program</label>
                <input type="text" name="sehat_sahulat_patient_id" id="sehat_sahulat_patient_id"
                    value="{{ old('sehat_sahulat_patient_id') }}"
                    class="w-full px-3 py-2 border rounded-md text-gray-700 focus:outline-none focus:border-blue-500"
    ...
    @elseif($isGovernment)
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 ">
            <div class="mb-4">
                <label for="government_card_no" class="block text-red-500 font-bold mb-2">Card No (CAUTION: Card no wil be
                    audited)</label>
                @if(request()->routeIs('patient.edit'))
                    <input type="text" name="government_card_no" value="{{$patient->government_card_no}}"
                        id="government_card_no"
                        class="w-full px-3 py-2 border rounded-md text-gray-700 focus:outline-none focus:border-blue-500"
                        placeholder="Government card number">
                @else
                    <input type="text" name="government_card_no" id="government_card_no"
                        class="w-full px-3 py-2 border rounded-md text-gray-700 focus:outline-none focus:border-blue-500"
                        placeholder="Government card number">
                @endif
            </div>
            <div class="mb-4">
                ...
                @if(request()->routeIs('patient.edit'))
                    <input type="text" value="{{ $patient->designation }}" name="designation" id="designation"
                        class="w-full px-3 py-2 border rounded-md text-gray-700 focus:outline-none focus:border-blue-500"
                        placeholder="Government designation">
                @else
...
@section('custom_script')
    <script>
        ...
        // This partial is also rendered on pages without cnic/mobile/first_name fields (e.g. issue-new-chit).
        ...
        document.addEventListener("DOMContentLoaded", function () {
            // Set focus on the "first_name" input field, if present on this page
