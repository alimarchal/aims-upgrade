                    ...
                    <table class="table-auto w-full border-collapse border border-black"
                        ...
                        <thead>
                            <tr class="border-black">
                                ...
                                <th class="border-black border px-4 py-2 text-center hidden print:block" rowspan="2">
                                    Signature</th>
                            </tr>
                            <tr class="border-black">
                        ...
                        <tbody>
                                ...
                                <tr class="border-black">
                                    ...
                                    <td class="border-black border px-4 py-2 text-center">
                                        @if(request()->has('start_date') || request()->has('end_date'))
                                            <a href="{{ route('invoice.issued', ['start_date' => request()->input('start_date'), 'end_date' => request()->input('end_date'), 'filter[user_id]' => $key, 'filter[government_non_government]' => 1]) }}"
                                                class="text-blue-500 hover:underline">{{ number_format($value['Invoices Entitled'], 0) }}</a>
                                        @else
                                            <a href="{{ route('invoice.issued', ['start_date' => \Carbon\Carbon::now()->format('Y-m-d'), 'end_date' => \Carbon\Carbon::now()->format('Y-m-d'), 'filter[user_id]' => $key, 'filter[government_non_government]' => 1]) }}"
                                                class="text-blue-500 hover:underline">{{ number_format($value['Invoices Entitled'], 0) }}</a>
                                        @endif
                                    ...
                                    <td class="border-black border px-4 py-2 text-center">
                                        @if(request()->has('start_date') || request()->has('end_date'))
                                            <a href="{{ route('invoice.issued', ['start_date' => request()->input('start_date'), 'end_date' => request()->input('end_date'), 'filter[user_id]' => $key, 'filter[government_non_government]' => 0]) }}"
                                                class="text-blue-500 hover:underline">{{ number_format($value['Invoices Non Entitled'], 0) }}</a>
                                        @else
                                            <a href="{{ route('invoice.issued', ['start_date' => \Carbon\Carbon::now()->format('Y-m-d'), 'end_date' => \Carbon\Carbon::now()->format('Y-m-d'), 'filter[user_id]' => $key, 'filter[government_non_government]' => 0]) }}"
                                                class="text-blue-500 hover:underline">{{ number_format($value['Invoices Non Entitled'], 0) }}</a>
                                        @endif

                                    </td>
                                    <td class="border-black border px-4 py-2 text-center">
                                        {{ number_format($value['Invoices HIF'], 2) }}</td>
                                    <td class="border-black border px-4 py-2 text-center">
                                        {{ number_format($value['Invoices'] - $value['Invoices HIF'], 2) }}</td>
                                    <td class="border-black border px-4 py-2 text-center">
                                        {{ number_format($value['Invoices'], 2) }}</td>

                                    <td class="border-black border px-4 py-2 text-center">
                                        @if(request()->has('start_date') || request()->has('end_date'))
                                            <a href="{{ route('chits.issued', ['start_date' => request()->input('start_date'), 'end_date' => request()->input('end_date'), 'filter[user_id]' => $key, 'filter[government_non_gov]' => 1]) }}"
                                                class="text-blue-500 hover:underline">{{ number_format($value['Chit Entitled'], 0) }}</a>
                                        @else
                                            <a href="{{ route('chits.issued', ['start_date' => \Carbon\Carbon::now()->format('Y-m-d'), 'end_date' => \Carbon\Carbon::now()->format('Y-m-d'), 'filter[user_id]' => $key, 'filter[government_non_gov]' => 1]) }}"
                                                class="text-blue-500 hover:underline">{{ number_format($value['Chit Entitled'], 0) }}</a>
                                        @endif
                                    </td>
                                    <td class="border-black border px-4 py-2 text-center">
                                        @if(request()->has('start_date') || request()->has('end_date'))
                                            <a href="{{ route('chits.issued', ['start_date' => request()->input('start_date'), 'end_date' => request()->input('end_date'), 'filter[user_id]' => $key, 'filter[government_non_gov]' => 0]) }}"
                                                class="text-blue-500 hover:underline">{{ number_format($value['Chit Non Entitled'], 0) }}</a>
                                        @else
                                            <a href="{{ route('chits.issued', ['start_date' => \Carbon\Carbon::now()->format('Y-m-d'), 'end_date' => \Carbon\Carbon::now()->format('Y-m-d'), 'filter[user_id]' => $key, 'filter[government_non_gov]' => 0]) }}"
                                                class="text-blue-500 hover:underline">{{ number_format($value['Chit Non Entitled'], 0) }}</a>
                                        @endif

                                    </td>
                                    <td class="border-black border px-4 py-2 text-center">
                                        {{ number_format($value['Chits HIF'], 2) }}</td>
                                    <td class="border-black border px-4 py-2 text-center">
                                        {{ number_format($value['Chits'] - $value['Chits HIF'], 2) }}</td>
                                    <td class="border-black border px-4 py-2 text-center">
                                        {{ number_format($value['Chits'], 2) }}</td>
                                    <td class="border-black border px-4 py-2 text-center">
                                        {{ number_format($value['Chits'] + $value['Invoices'], 2)  }}</td>
                            ...
                            <tr class="border-black">
                                ...
                                <td class="border-black border px-4 py-2 text-center font-bold">
                                    {{ number_format($inv_non_entitled, 0) }}</td>
                                <td class="border-black border px-4 py-2 text-center font-bold">
                                    {{ number_format($inv_entitled, 0) }}</td>
                                <td class="border-black border px-4 py-2 text-center font-bold">
                                    {{ number_format($totalInvoicesHIF, 2) }}</td>
                                <td class="border-black border px-4 py-2 text-center font-bold">
                                    {{ number_format($totalInvoices - $totalInvoicesHIF, 2) }}</td>
                                <td class="border-black border px-4 py-2 text-center font-bold">
                                    {{ number_format($totalInvoices, 2) }}</td>
                                <td class="border-black border px-4 py-2 text-center font-bold">
                                    {{ number_format($chit_entitled, 0) }}</td>
                                <td class="border-black border px-4 py-2 text-center font-bold">
                                    {{ number_format($chit_non_entitled, 0) }}</td>
                                <td class="border-black border px-4 py-2 text-center font-bold">
                                    {{ number_format($totalChitHIF, 2) }}</td>
                                <td class="border-black border px-4 py-2 text-center font-bold">
                                    {{ number_format($totalChits - $totalChitHIF, 2) }}</td>
                                <td class="border-black border px-4 py-2 text-center font-bold">
                                    {{ number_format($totalChits, 2) }}</td>
                                <td class="border-black border px-4 py-2 text-center font-bold">
