<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex justify-center items-center float-right">
            <a href="{{route('patient.create-opd')}}"
                class="mx-2 float-right inline-flex items-center px-4 py-2 bg-red-800 border border-transparent
            ...
            <a href="{{ route('patient.create') }}"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="#000000"
            ...
            <form action="">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="filter[id]" value="{{ request('filter.id') }}" id="id"
                            class="w-full px-3 py-2 border rounded-md text-gray-700 focus:outline-none focus:border-blue-500"
                            placeholder="Patient ID Type here">
                    </div>

                    <div>
                        <label for="first_name" class="block text-gray-700 font-bold mb-2">Name</label>
                        <input type="text" name="filter[first_name]" value="{{ request('filter.first_name') }}"
                            id="first_name"
                            class="w-full px-3 py-2 border rounded-md text-gray-700 focus:outline-none focus:border-blue-500"
                            placeholder="Enter name">
                    </div>
                    <div>
                        <label for="father_husband_name"
                            class="block text-gray-700 font-bold mb-2">Father/Son/Do</label>
                        <input type="text" name="filter[father_husband_name]"
                            value="{{ request('filter.father_husband_name') }}" id="father_husband_name"
                            class="w-full px-3 py-2 border rounded-md text-gray-700 focus:outline-none focus:border-blue-500"
                    ...
                    <div>
                        ...
                        <input type="text" name="filter[cnic]" id="cnic" value="{{ request('filter.cnic') }}"
                            class="w-full px-3 py-2 border rounded-md text-gray-700 focus:outline-none focus:border-blue-500"
                            placeholder="Enter CNIC (00000-0000000-0)">
                    </div>
                    <div>
                        <label for="mobile" class="block text-gray-700 font-bold mb-2">Mobile No.</label>
                        <input type="text" name="filter[mobile]" id="mobile_no" value="{{ request('filter.mobile') }}"
                            class="w-full px-3 py-2 border rounded-md text-gray-700 focus:outline-none focus:border-blue-500"
                                ...
                                <tr class="border-black">
                                    ...
                                    <td class="border-black border px-4 py-2 text-center">
                                        ...
                                        <a href="{{ route('patient.actions', $patient->id) }}">
                                            <img src="{{ Storage::url('settings.png') }}" alt="actions"
                                                class="w-8 inline  hover:scale-110">
                                        </a>

                                        {{-- <a href="{{ route('patient.issue-new-chit', $patient->id) }}">--}}
                                            {{-- <img src="{{ Storage::url('new_2.png') }}" alt="actions"
                                                class="w-6 inline  hover:scale-110">--}}
                                            {{-- </a>--}}
